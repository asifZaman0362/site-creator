...
let [paddingLeftInput, paddingRightInput, paddingTopInput, paddingBottomInput] =
  [null, null, null, null];
let [marginLeftInput, marginRightInput, marginTopInput, marginBottomInput] = [
  null,
  null,
  null,
  null,
...
function hoverStopped() {
  if (templateList) templateList.classList.remove("shown");
}
...
function dragOver(ev) {
  ev.preventDefault();
  if (ev.dataTransfer.effectAllowed == "copy") dropEffect = "copy";
  else if (ev.dataTransfer.effectAllowed == "move") dropEffect = "move";
}
...
function updateBorderRadiusInput() {
  ...
  sel("#border-radius-input").value =
    context.element.style.borderRadius.match(/[0-9]+/);
  sel("#top-left-radius").value = getComputedStyle(context.element)
    .borderTopLeftRadius
    ? parseInt(getComputedStyle(context.element).borderTopLeftRadius)
    : 0;
  sel("#top-right-radius").value = getComputedStyle(context.element)
    .borderTopRightRadius
    ? parseInt(getComputedStyle(context.element).borderTopRightRadius)
    : 0;
  sel("#bottom-right-radius").value = getComputedStyle(context.element)
    .borderBottomRightRadius
    ? parseInt(getComputedStyle(context.element).borderBottomRightRadius)
    : 0;
  sel("#bottom-left-radius").value = getComputedStyle(context.element)
    .borderBottomLeftRadius
    ? parseInt(getComputedStyle(context.element).borderBottomLeftRadius)
    : 0;
}

function updateBorderInspector() {
  borderInspector.classList.remove("checked");
  sel("#border-checkbox").checked =
    !context.element.classList.contains("border-off");
...
function updateBackgroundColorInput() {
  ...
  sel("#bg-color").value = rgbStringToHex(
    getComputedStyle(context.element).backgroundColor
  );
  sel("#bg-color-hex").value = rgbStringToHex(
    getComputedStyle(context.element).backgroundColor
  );
}

function updateColorInput() {
  sel("#fg-color").value = rgbStringToHex(
    getComputedStyle(context.element).backgroundColor
  );
  sel("#fg-color-hex").value = rgbStringToHex(
    getComputedStyle(context.element).backgroundColor
  );
}
...
function updateBackgroundImageInput() {
  ...
  sel("#background-image-checkbox").checked =
    !context.element.classList.contains("background-image-off");
...
function updateTextInspector() {
  ...
  const computedDirection = window.getComputedStyle(
    context.element.parentElement || document.body
  ).direction;
...
function updateShadowInspector(text) {
  ...
  if (text) sel("#shadow-label").innerHTML = "Text Shadow";
  else sel("#shadow-label").innerHTML = "Box Shadow";
  sel("#shadow-checkbox").checked =
    !context.element.classList.contains("shadow-off");
  let shadow = text
    ? getComputedStyle(context.element).textShadow
    : getComputedStyle(context.element).boxShadow;
...
function updateContainerSizeInspector() {
  ...
  sel("#container-size-input").value = parseInt(
    getComputedStyle(projectRoot).width
  );
}
...
function updateInspector(event) {
  ...
  switch (context.type) {
    ...
    case "MENU": {
      ...
      if (context.element.children.length > 0) showChildren();
      break;
    }
    case "UL":
    case "OL": {
    ...
    case "LI": {
      ...
      updateUrlInspector();
      if (
        context.element.children.length == 0 ||
        hasEditableChildren(context.element.children)
      ) {
...
function startDrag(ev) {
  ...
  ev.dataTransfer.setData("text", element.outerHTML);
}
...
function onShadowUpdate() {
  let color = hexToRgb(
    sel("#shadow-color-hex").value,
    sel("#shadow-color-opacity").value
  );
...
function onUnderlineCheck(input) {
  ...
  if (context.element.style.textDecoration == "underline") {
    ...
    input.classList.add("checked");
    document
      .querySelector("#strikethrough-checkbox")
...
function onStyleChangeUpdate(value, attribute, unit, element) {
  ...
  if (context.element) {
    ...
    } else context.element.style[attribute] = newValue;
  }
}
...
function showHighlightElement(element) {
  ...
  let height =
    rect.height + parseInt(styles.marginTop) + parseInt(styles.marginBottom);
