...
class Copy_With_Style {
  ...
  ******************************************************************************************/
  ...
  constructor({
    ...
    if (this.debug)
      console.log(
        `Configuring progress: ${progress instanceof HTMLElement}, ${progress.tagName
        }, ${button.parentElement.querySelector("progress")}, ${this.show_progress
        }`
      );
  ...
  async prepare_copy_when_ready(fingerprint = "no fingerprint") {
    async function prepare_the_copy() {
      if (await this.#ready_to_prepare(fingerprint)) {
        if (this.debug)
          console.log(
            `${fingerprint} Rendering is complete. Requesting prepare_copy() ... (this.#bail: ${this.#bail
  ...
  async prepare_copy() {
    ...
    if (this.debug)
      console.log(
        `prepare_copy finished: ${element.id}, produced: ${this.HTML ? this.HTML.length : 0
        } characters of HTML amd ${this.text ? this.text.length : 0
        } characters of text.`
      );
  }
  ...
  ******************************************************************************************/
  ...
  #schedule_preparation() {
    async function handler() {
      if (this.debug)
        console.log(
          `Scheduled copy preparation triggered ... triggers: ${this.triggers
          }, preparations: ${this.#is_being_prepared}, ${this.#is_prepared
  ...
  #schedule_observation() {
    async function handler() {
      if (this.debug)
        console.log(
          `Scheduled observer triggered ... triggers: ${this.triggers
          }, observe? ${this.observe}, preparations: ${this.#is_being_prepared
          }, ${this.#is_prepared}, ready state: ${document.readyState
          }`
        );
      if (
        !this.#is_prepared &&
